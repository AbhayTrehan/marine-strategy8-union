...
def test_classify_image_candidates_empty():
    gmm = _make_fitted_gmm()
    o_pos, o_neg, gammas = classify_image_candidates([], gmm, tau=0.5)
...
def test_tau_threshold_moves_objects_between_sets():
    ...
    _, _, gammas = classify_image_candidates(candidates, gmm, tau=0.5)
    gamma = gammas[0]
    o_pos_low, o_neg_low, _ = classify_image_candidates(candidates, gmm, tau=max(0.0, gamma - 0.2))
    o_pos_high, o_neg_high, _ = classify_image_candidates(candidates, gmm, tau=min(1.0, gamma + 0.2))
...
def test_build_question_file_chair_format():
    ...
    gmm = _make_fitted_gmm()
    questions, per_image = build_question_file(qpath, cache, gmm, tau=0.5)
...
def test_build_question_file_image_filter():
    ...
    gmm = _make_fitted_gmm()
    questions, per_image = build_question_file(qpath, cache, gmm, tau=0.5, image_filter=["img0.jpg"])
...
def test_build_question_file_missing_image_in_cache_treated_as_no_candidates():
    ...
    gmm = _make_fitted_gmm()
    questions, per_image = build_question_file(qpath, {}, gmm, tau=0.5)
    assert len(questions) == 1
